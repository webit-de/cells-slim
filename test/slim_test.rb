...
class SlimTest < MiniTest::Spec
  ...
  # content_tag { }
  it { (song_cell.(:with_content_tag) + '<br>').must_equal "<div>Beachparty</div><br>" }

  # content_tag { content_tag { } }
  it do song_cell.(:with_content_tag_and_content_tag).must_equal %{<span>Title:<div>Still Knee Deep</div></span>}
  end

  it { (song_cell.(:with_new_tag_builder) + '<br>').must_equal "<div>rails 5 style</div><br>" }
  ...
  it( "xxx") do
    ...
    form_for_tag = "<form class=\"new_open\" id=\"new_open\" action=\"/\" accept-charset=\"UTF-8\" data-remote=\"true\" method=\"post\"><input name=\"utf8\" type=\"hidden\" value=\"&#x2713;\" /><input type=\"hidden\" name=\"_method\" value=\"patch\" /><input type=\"text\" name=\"open[id]\" id=\"open_id\" />"
    form_for_tag = "<form accept-charset=\"UTF-8\" action=\"/\" class=\"new_open\" id=\"new_open\" data-remote=\"true\" method=\"post\"><div style=\"margin:0;padding:0;display:inline\"><input name=\"utf8\" type=\"hidden\" value=\"&#x2713;\" /><input type=\"hidden\" name=\"_method\" value=\"patch\" /></div><input id=\"open_id\" name=\"open[id]\" size=\"30\" type=\"text\" />" if ActionPack::VERSION::MAJOR == 3
...
<div><p>Concat!</p>Whoo</div>
  ...
  describe "block capture" do
    it do
      (song_cell.(:with_capture) + '<br>').must_equal %{Hallo<p>More</p>Hidden!<br>}
    end
  end
